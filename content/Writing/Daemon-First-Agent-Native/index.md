...
date: 2026-04-19
description: Three projects in different domains tested the same daemon-first architecture. This is what held up, and what remains aspirational.
tags:
  - architecture
  - agents
  - pattern
...
CLI and web parity establishes the conditions for agent parity. Every daemon operation gets a CLI command with machine-first output. This isn't about serving terminal-native users, though it does that. It's about making the application controllable by any agent with shell access, without a custom SDK or source-code reading. It makes agent-composed features possible; it does not mean they are already present.
...
- Every's [agent-native guide](https://every.to/guides/agent-native), an external formulation of the agent-native concept
